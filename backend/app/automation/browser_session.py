...
workers must never open the same profile at the same time â€” enforced with
...
Environment:
  ...
  * If no display (typical cloud/preview environment), the browser is launched
    headless â€” which means an operator cannot complete manual login there.
...
def _has_display() -> bool:
    # Windows desktop sessions have a GUI even though DISPLAY is not set.
    if os.name == "nt":
        return True
...
# ---------------------------------------------------------------------------
async def _verify_persistent_profile(profile_path: Path, expected_username: str = "") -> str:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                # network hiccup â€” fall through to cookie check anyway
                pass
            await asyncio.sleep(2)  # let redirects settle

            # 1) Cookie signal â€” the sessionid cookie is the auth token.
            ...
            # 2) URL / DOM signal â€” if Instagram redirected to /accounts/login/
            ...
            if not has_session_cookie or redirected_to_login or login_form_present:
                return "login_required"

            # SAFETY: verify that this persistent profile belongs to the
            # Instagram account stored in our database.
            if expected_username:
                ...
                try:
                    ...
                    # Fail closed: if identity cannot be proven, do not allow posting.
                    return "identity_mismatch"
    ...
    async with lock:
            ...
            try:
                ...
                log(
                    db,
                    "INFO",
                    f"Launching browser for '{account_name}' â€” waiting for manual login",
            ...
            try:
                ...
                async with async_playwright() as p:
                    ...
                    if not has_display:
                        ...
                        _write_log(
                            account_id,
                            "WARNING",
                            "This host has no display â€” visible manual login requires running the app locally.",
                        )
                        return
                ...
                status = await _verify_persistent_profile(profile_path, account_name)
                _update_account(
                    account_id,
                ...
                _write_log(
                    account_id,
                    "ERROR",
                    f"Connect session error: {repr(e)}\n{__import__('traceback').format_exc()}",
                )
            ...
            try:
                if not profile_path or not profile_path.exists() or not any(profile_path.iterdir()):
                    ...
                    _write_log(
                        account_id,
                        "WARNING",
                        "No persistent profile yet â€” Connect Instagram first",
                    )
                    return
                status = await _verify_persistent_profile(profile_path, account_name)
                _update_account(
...
# ---------------------------------------------------------------------------
# Environment probe â€” used by the UI to show a helpful banner
# ---------------------------------------------------------------------------
def get_environment() -> dict:
    ...
    if not PLAYWRIGHT_AVAILABLE:
        ...
        msg = (
            ...
            "To connect an Instagram account, run the app locally and click Connect Instagram â€” "
            "the browser will open on your desktop."
        )
        level = "warn"
    else:
        msg = "Ready â€” clicking Connect Instagram will open a browser window on this machine for manual login."
        level = "ok"
    return {
